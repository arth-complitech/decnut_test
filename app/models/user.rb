class User < ActiveRecord::Base
  ...
  belongs_to :group
  has_many :favorites
  has_many :favorite_pathways, through: :favorites, source: :favorited, source_type: 'Pathway'
  ...
  ## Scope
  scope :user_except_content_admin, -> (user) { where('type NOT IN (?) and group_id in (?)', ["ContentAdmin","SuperAdmin"], user.group.id) }
  scope :users_from_same_group, -> (user) { where('group_id in (?)', user.group.id)}
